...
DENSE_RANK() OVER(PARTITION BY TRANSACTIONTYPE ORDER BY AMOUNT DESC) AS
TRANSACTION_RANK
FROM TRANSACTIONS;
...
SELECT LOANID,CUSTOMERID,LOANAMOUNT,
ROW_NUMBER() OVER(PARTITION BY CUSTOMERID ORDER BY LOANAMOUNT DESC) AS RN
FROM LOANS;
...
-- CUSTOMERS WITH TOTAL BALANCE GREATER THAN 50000
...
SELECT *
FROM LOANS
WHERE INTERESTRATE > (SELECT AVG(INTERESTRATE)
					FROM LOANS);
...
-- CUSTOMERS WITH ATLEAST ONE ACCOUNT AND ACCOUNT
